...
The system progresses through a state machine with 5 distinct phases:

```typescript
type SchedulingPhase =
  | "WAITING_FOR_A_REPLY"       // Initial proposal sent to User A
  | "WAITING_FOR_A_ALTERNATIVE"  // A declined, needs to propose new time
  | "WAITING_FOR_B_REPLY"       // Time forwarded to User B for approval
...
```
WAITING_FOR_A_REPLY
  ├─ A accepts → WAITING_FOR_B_REPLY
  ├─ A proposes alternative → WAITING_FOR_B_REPLY (update proposedSlot)
  └─ A declines without alternative → WAITING_FOR_A_ALTERNATIVE

WAITING_FOR_A_ALTERNATIVE
  └─ A proposes time → WAITING_FOR_B_REPLY

WAITING_FOR_B_REPLY
  ├─ B accepts → AGREED (generate venue, enable portal)
  ├─ B proposes alternative → WAITING_FOR_A_REPLY (update proposedSlot)
  └─ B declines without alternative → WAITING_FOR_A_ALTERNATIVE

WAITING_FOR_B_ALTERNATIVE
  └─ B proposes time → WAITING_FOR_A_REPLY
